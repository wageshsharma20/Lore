        ...
        {isBlocked && check.conflicts && check.conflicts.length > 0 && (
          <CardContent className="p-8">
            ...
            {check.conflicts.map((conflict, idx) => (
              <div key={idx} className="bg-gray-50 border border-gray-200 p-6 relative mb-6">
                <div className="absolute -top-3 left-6 bg-white px-2 text-sm text-gray-500 font-medium">Historical Record</div>
                
                <h4 className="text-lg font-bold mb-2">{conflict.decision_title}</h4>
                ...
                <div className="flex items-center gap-4 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="secondary" className="bg-gray-200 hover:bg-gray-200 text-gray-800 border-none shadow-sm">{conflict.decision_author}</Badge>
                  </div>
                  <div className="w-1 h-1 bg-gray-300"></div>
                  <span className="text-gray-500">Date: {conflict.decision_date}</span>
                  <div className="w-1 h-1 bg-gray-300"></div>
                  <Badge variant="outline" className="border-red-300 text-red-700 bg-white">Severity: {conflict.severity}</Badge>
                </div>
              </div>
            ))}
            ...
            <div className="mt-8 p-4 bg-amber-50 border border-amber-200 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-amber-700/90 text-sm mt-1">To proceed with this PR, you must get sign-off from the decision authors or submit a superseding Architecture Decision Record (ADR).</p>
              </div>
            </div>
          </CardContent>
        )}
